...
function check_norbind {
    ...
    /create-busybox-container "$_root"
    systemd-nspawn --bind /lib64 --bind /usr/lib64 --register=no -D "$_root" --bind=/tmp/binddir:/mnt:norbind /bin/sh -c 'CONTENT=$(cat /mnt/subdir/file); if [[ $CONTENT != "outer" ]]; then echo "*** unexpected content: $CONTENT"; return 1; fi'
}
...
function run {
    ...
    if SYSTEMD_NSPAWN_UNIFIED_HIERARCHY="$1" SYSTEMD_NSPAWN_USE_CGNS="$2" SYSTEMD_NSPAWN_API_VFS_WRITABLE="$3" systemd-nspawn  --bind /lib64 --bind /usr/lib64--register=no -D "$_root" "$_netns_opt" --network-veth -b; then
       return 1
    fi
